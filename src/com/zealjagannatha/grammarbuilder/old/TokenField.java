...
public class TokenField {
	...
	public TokenField(Field field) {
		this.field = field;
		Annotation preToken = field.getAnnotation(Symbol.class);
		if (!(preToken instanceof Symbol))
			throw new RuntimeException("Attempt to create TokenField from non-token field: " + field.getName());
		token = (Symbol) preToken;
	}
	...
	private <K> void nextTokenList(String padding, ParserLookaheadStream stream, Class<K> clazz) throws IOException {
		boolean cont = true;
		while (cont) {
			if (clazz == String.class) {
				if (stream.nextToken() == null) {
